...
class BusLSTM(nn.Module):
    ...
    def __init__(self,
        ...
        # C. Bridging the two networks

        self.fc_h = nn.Linear(encoder_hidden_size, num_lstm_layers * lstm_hidden_size)
        self.fc_c = nn.Linear(encoder_hidden_size, num_lstm_layers * lstm_hidden_size)
        ...
        decoder_input_size = n_x2_dense_features + x2_total_emb_dim
    ...
    def forward(self, x1_cat, x1_dense, x2_cat, x2_dense):
        ...
        # C. Generating h_0 and c_0 for Decoder

        batch_size = x1_dense.size(0)
        h_0 = self.fc_h(context).view(self.num_lstm_layers, batch_size, self.lstm_hidden_size)
        c_0 = self.fc_c(context).view(self.num_lstm_layers, batch_size, self.lstm_hidden_size)
        ...
        decoder_input = torch.cat([x2_dense, x2_emb_concat], dim=2)
